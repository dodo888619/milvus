...
class ResponseChecker:
    ...
    def check_value_equal(res, func_name, params):
        ...
        if func_name == "list_connections":
            if not isinstance(res, list):
                log.error(
                    f"[CheckFunc] Response of list_connections is not a list: {str(res)}"
                )
                assert False
            ...
            if not isinstance(list_content, list):
                log.error(
                    f"[CheckFunc] Check param of list_content is not a list: {str(list_content)}"
                )
                assert False
    ...
    def check_search_results(search_res, func_name, check_items):
        ...
        if func_name != 'search':
            log.warning(f"The function name is {func_name} rather than search")
        if len(check_items) == 0:
            raise Exception("No expect values found in the check task")
        ...
        for hits in search_res:
            ...
            if (len(hits) != check_items["limit"]) \
                ...
                if check_items.get("ids", None) is not None:
                    ...
                    searched_original_vectors.extend(
                        check_items["original_vectors"][id] for id in hits.ids
                    )
    ...
    def check_search_iterator(search_res, func_name, check_items):
        ...
        if func_name != 'search_iterator':
            log.warning(f"The function name is {func_name} rather than search_iterator")
    ...
    def check_query_results(query_res, func_name, check_items):
        ...
        """
        if func_name != 'query':
            log.warning(f"The function name is {func_name} rather than query")
    ...
    def check_query_iterator(query_res, func_name, check_items):
        ...
        if func_name != 'query_iterator':
            log.warning(f"The function name is {func_name} rather than query_iterator")
        ...
        while True:
            ...
            if len(res) == 0:
                ...
                break
            pk_list.extend(res[i][ct.default_int64_field_name] for i in range(len(res)))
            if check_items.get("limit", None):
    ...
    def check_distance(distance_res, func_name, check_items):
        if func_name != 'calc_distance':
            log.warning(f"The function name is {func_name} rather than calc_distance")
